...
export default function SignUp() {
  ...
  const [legalType, setLegalType] = useState("terms"); // "terms" | "privacy"
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // 2. Successful Registration logic
      // After signup, we require email OTP verification before accessing the dashboard.
      sessionStorage.setItem("pending_verify_email", email);
      // If backend returns something like { verification_required: true }
      if (result?.verification_required || result?.mfa_required || result?.email_verification_required) {
    ...
    <>
      <AuthLayout title="Sign Up" subtitle="Enter your email and password to sign up!">
        ...
        <form className={form.form} onSubmit={onSubmit}>
          ...
          <label className={page.termsRow}>
            ...
            <span>
              ...
              <button
                ...
                onClick={() => { setLegalType("terms"); setLegalOpen(true); }}
              >
              ...
              <button
                type="button"
                className={page.inlineLink}
                onClick={() => { setLegalType("privacy"); setLegalOpen(true); }}
              >
                Privacy Policy
              </button>
            </span>
          ...
          {error && (
            <pre style={{ color: "crimson", fontSize: 13, margin: 0, whiteSpace: "pre-wrap" }}>
              {error}
            </pre>
          )}
          ...
          <p className={form.bottomText}>
            Already have an account? <a className={form.link} href="/signin">Sign In</a>
          </p>
        </form>
      </AuthLayout>
      ...
      <LegalModal
        open={legalOpen}
        title={legalType === "terms" ? "Terms and Conditions" : "Privacy Policy"}
